...
export function CareerForm({
  ...
  const onSubmit = async (data: Values) => {
    ...
    console.log("Application:", { ...data, resume: file });
    setDone(true);
  };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 rounded-4xl border border-border bg-card p-6 md:p-8">
      {disabled && (
        <div className="flex items-center gap-3 rounded-2xl border border-brand/30 bg-brand/5 px-4 py-3 text-sm text-muted-foreground">
          ...
          Applications are paused while there are no open roles — check back soon.
        </div>
      )}
      <fieldset disabled={disabled} className="flex flex-col gap-5 disabled:opacity-50">
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="flex flex-col gap-2">
          <span className="text-sm font-medium">Name</span>
          <input {...register("name")} className={input} placeholder="Your name" />
          {errors.name && <span className="text-xs text-brand">{errors.name.message}</span>}
        </label>
        <label className="flex flex-col gap-2">
          <span className="text-sm font-medium">Email</span>
          <input {...register("email")} className={input} placeholder="you@email.com" />
          {errors.email && <span className="text-xs text-brand">{errors.email.message}</span>}
        </label>
      </div>
      <label className="flex flex-col gap-2">
        ...
        {errors.role && <span className="text-xs text-brand">{errors.role.message}</span>}
      </label>
      <label className="flex flex-col gap-2">
        <span className="text-sm font-medium">Portfolio / LinkedIn (optional)</span>
        <input {...register("portfolio")} className={input} placeholder="https://" />
        {errors.portfolio && <span className="text-xs text-brand">{errors.portfolio.message}</span>}
      </label>
      <label className="flex flex-col gap-2">
        <span className="text-sm font-medium">Why Ezura Arc?</span>
        <textarea {...register("message")} rows={4} className={`${input} resize-none`} placeholder="Tell us what draws you to the studio." />
        {errors.message && <span className="text-xs text-brand">{errors.message.message}</span>}
      </label>
